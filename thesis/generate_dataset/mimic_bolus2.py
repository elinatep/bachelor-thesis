...
def get_bolus(conn_params, stay_id):
    
    bolus_query = f"SELECT * FROM mimiciv_icu.inputevents WHERE ordercategoryname = '03-IV Fluid Bolus' AND stay_id = {stay_id}"
    bolus_result = get_sql_data(conn_params, bolus_query)
    

    bolus_flag = bolus_result.shape[0] > 0
    
    return bolus_flag

def get_bolus_time(conn_params, stay_id):
    # Query the database to get the timestamp of the first bolus administration
    bolus_time_query = f"SELECT MIN(starttime) FROM mimiciv_icu.inputevents WHERE ordercategoryname = '03-IV Fluid Bolus' AND stay_id = {stay_id}"
    bolus_time_result = get_sql_data(conn_params, bolus_time_query)
    
    # Extract the timestamp if any record was found
    bolus_time = bolus_time_result.iloc[0, 0] if bolus_time_result.shape[0] != 0 else -1
...
def generate_labels(data_path, cpu):
    db_params = {
            "host"      : "129.132.40.54",
            "database"  : "mimic",
            "user"      : "mgiordano",
    ...
    with open(os.path.join(data_path, "0labels.txt"), 'w') as f:
        f.write("id,sepsis,sepsis_time,time_to_sepsis,weight,height,age,gender,ethnicity,bolus,shock_time,time_to_shock,sepsis_to_shock\n")
    
    # Loop through each batch
    for batch in dataset_generator:
        pbar.update(dataloader_params["batch_size"])
        wandb.log({"Generated patients": pbar.n})
        for stay_id, wind, ons, time_to_sepsis, inc, weight, height, age, gender, ethnicity, bolus, shock_time, time_to_shock, sepsis_to_shock in batch:
            # Check if inclusion criteria met and if sepsis and bolus are true
            if inc == 0 and ons != -1 and bolus:
                with open(os.path.join(data_path, "0labels.txt"), 'a') as f:
                    f.write(f"{stay_id},{ons != -1},{ons},{time_to_sepsis},{weight},{height},{age},{gender},{ethnicity},{bolus},{shock_time},{time_to_shock},{sepsis_to_shock}\n")
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Getting data for the patient. It is not guaranteed to have data
        ...
        bolus = get_bolus(self.conn_params, stay_id)
        shock_time = get_bolus_time(self.conn_params, stay_id)
        ...
        if sepsis3 != -1:
            ...
            if shock_time is not None:
                if sepsis3 is not None and time_to_sepsis is not None:
                    time_to_shock = shock_time - (sepsis3 - time_to_sepsis)
                if sepsis3 is not None and shock_time > sepsis3:
                    sepsis_to_shock = shock_time - sepsis3

        return (stay_id, vitals, sepsis3, time_to_sepsis, inclusion, weight, height, age, gender, ethnicity, bolus, shock_time, time_to_shock, sepsis_to_shock)
